...
//
// This program by Xiao Hu (z5223731) on 28/03/2019
...
int count_arkles(int tiles1[N_TILES], int tiles2[N_TILES]) {
    int i = 0, j = 0, n = 0, tmp[N_TILES], tmp2[N_TILES];
    while (i < N_TILES) {
        tmp[i] = tiles1[i];
        tmp2[i] = tiles2[i];
        i++;
    ...
    while (i < N_TILES) {
        j = 0;
        while (j < N_TILES) {
            if (tmp[i] == tmp2[j] && j != i && tmp[j] != tmp2[j]) {
                n++;
                tmp[i] = 0;
                tmp2[j] = -1;
        }
...
void play_farnarkle(int hidden_tiles[N_TILES]) {
    ...
    for (a = 0; a < N_TILES; a++) {
        tiles2[a] = hidden_tiles[a];
    }
    while (i == 0) {
       printf("Enter guess for turn %d: ", j);
       read_tiles(tiles1);
       count_farnarkles(tiles1, tiles2);     
       count_arkles(tiles1, tiles2);
       printf("%d farnarkles, %d arkles\n", count_farnarkles(tiles1, 
               tiles2), count_arkles(tiles1, tiles2));
      
       if (count_farnarkles(tiles1, tiles2) == N_TILES) {
            printf("You win\n");
            i = -1;
        }
        j++;
